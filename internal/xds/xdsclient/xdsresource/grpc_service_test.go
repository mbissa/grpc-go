...
func TestParseGRPCServiceConfig_InvalidHeaderValueCharacters(t *testing.T) {
	insecurePlugin := &anypb.Any{
		TypeUrl: "type.googleapis.com/envoy.extensions.grpc_service.channel_credentials.insecure.v3.InsecureCredentials",
	}
	protoMsg := &v3corepb.GrpcService{
	...
	_, err := ParseGRPCServiceConfig(protoMsg, true, nil)
	if err == nil || !strings.Contains(err.Error(), "header value contains invalid character") {
		t.Fatalf("ParseGRPCServiceConfig() returned error: %v, want error with 'header value contains invalid character'", err)
	}
}
...
func TestParseGRPCServiceConfig_CallCredentials(t *testing.T) {
	insecurePlugin := &anypb.Any{
		TypeUrl: "type.googleapis.com/envoy.extensions.grpc_service.channel_credentials.insecure.v3.InsecureCredentials",
	}
	tokenProto, _ := anypb.New(&access_tokenpb.AccessTokenCredentials{Token: "my-access-token"})
	protoMsg := &v3corepb.GrpcService{
		TargetSpecifier: &v3corepb.GrpcService_GoogleGrpc_{
			GoogleGrpc: &v3corepb.GrpcService_GoogleGrpc{
				TargetUri:                "dns:///my-service:443",
				ChannelCredentialsPlugin: []*anypb.Any{insecurePlugin},
				CallCredentialsPlugin:    []*anypb.Any{tokenProto},
			},
		},
	}
	...
	if err := json.Unmarshal([]byte(cfg.CallCredentials), &callCreds); err != nil {
		t.Fatalf("Failed to unmarshal CallCredentials: %v", err)
	}
	if len(callCreds) != 1 {
		t.Fatalf("CallCredentials count got: %d, want: 1", len(callCreds))
	}
	if callCreds[0].Type != "access_token" {
		t.Errorf("CallCredentials.Type got: %q, want: %q", callCreds[0].Type, "access_token")
	}
	var tokenCfg map[string]string
	if err := json.Unmarshal(callCreds[0].Config, &tokenCfg); err != nil {
		t.Fatalf("Failed to unmarshal call creds config: %v", err)
	}
	if tokenCfg["token"] != "my-access-token" {
